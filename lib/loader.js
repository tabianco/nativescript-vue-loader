...
module.exports = function (content) {
  ...
  // <script>
  output += '/* script */\n'
  const script = parts.scripts.find(script => isNative ? script.attrs.native : script.attrs.web)
  if (script) {
    if (options.esModule) {
      output += script.src
        ? (
  ...
  // <template>
  output += '/* template */\n'
  const template = parts.templates.find(template => isNative ? template.attrs.native : template.attrs.web)
  if (template) {
    if (options.esModule) {
      output +=
        (template.src
