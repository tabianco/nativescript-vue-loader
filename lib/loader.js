...
module.exports = function (content) {
  ...
  const isNative = this.target !== 'web' && this.target !== 'node'
  ...
  // <script>
  output += '/* script */\n'
  const script = parts.scripts.find(script => isNative ? script.attrs.native : script.attrs.web) || parts.script
  if (script) {
    if (options.esModule) {
      output += script.src
        ? (
  ...
  // <template>
  output += '/* template */\n'
  const template = parts.templates.find(template => isNative ? template.attrs.native : template.attrs.web) || parts.template
  if (template) {
    if (options.esModule) {
      output +=
        (template.src
